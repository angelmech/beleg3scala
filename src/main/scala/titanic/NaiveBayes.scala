...
object NaiveBayes {
  ...
  /**
   *
   * Counts the number of occurences of each attribute value of one attribute
   *
   *  @param data    Data Set for counting
   *  @param attrib Attribute that should be counted
  ...
  def countAttributeValues(data:List[Map[String, Any]], attribList:String): Map[Any,Int]=
    data.map(x => x(attribList)).groupBy(x => x).view.mapValues(_.size).toMap
  ...
  def calcPriorPropabilities(data:List[Map[String, Any]], classAttrib:String):Map[Any,Double]=
    val classCounts = countAttributeValues(data, classAttrib)
    classCounts.map(x => (x._1, x._2.toDouble / data.size))
